...
class PHPCtags
{
	...
	$this->mParser = $factory->create(PhpParser\ParserFactory::ONLY_PHP7);
...
error_reporting(E_ALL);
    ...
    private function render($structure)
    {
        ...
        foreach ($structure as $struct) {
			 ...
			 */
			//$str .= $struct['name'];
			if (is_string($struct['name'])) {
				$str.= $struct['name'];
			} else {
				$str .= implode('\\', $struct['name']->getSubNodeNames());
			}
			/*
			if  ($struct['name'] instanceof PhpParser\Node\Name ){
				$str .= $struct['name']->toString;
			}else{
				$str .= $struct['name'];
			}
			 */
